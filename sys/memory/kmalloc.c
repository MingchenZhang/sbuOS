...
void* sf_realloc(void *ptr, size_t size) {
	...
	if(newRealSize > originalRealSize){// expand
		...
		if(totalAvailSize>=newRealSize){// merge a part of free block
			...
			memcpy(newSpace, originalHeader+1, originalHeader->requested_size);//not everything in block gets copied
		...
		memcpy(getFooter(freedHeader), freedHeader, sizeof(sf_header));//setup footer
...
void* sf_allocate(sf_free_header* freeHeader, size_t requiredSize) {
	...
	if(available8Bytes < real8Bytes) error("sf_allocate received insufficient space\n"); //debug
...
sf_free_header* sf_initializeFreeBlock(void* startPt, sf_free_header* lastBlock, size_t newPageSize) {
	...
	if(firstPageInitialized){
		...
		footer->requested_size = 0;
		memset(footer+1, 0, sizeof(sf_header));
	}else{
		header = (sf_free_header*)(((sf_header*)startPt)+1);
		...
		footer->requested_size = 0;
		memset(((sf_header*)header)-1, 0, sizeof(sf_header));
		memset(((sf_header*)footer)+1, 0, sizeof(sf_header));
	}
...
sf_free_header* sf_coalescing(sf_free_header* header, bool onlyForward) {
	...
	memcpy(nextFooter, newHeader, sizeof(sf_header));
